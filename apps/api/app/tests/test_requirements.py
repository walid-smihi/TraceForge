...
async def test_create_requirement(client: AsyncClient, project_id: str):
    resp = await client.post(
        ...
        json={"title": "User can login", "description": "The user must be able to login.", "req_type": "functional"},
    )
...
async def test_auto_increment_codes(client: AsyncClient, project_id: str):
    ...
    resp = await client.post(f"/api/v1/projects/{project_id}/requirements", json={"title": "Second"})
...
async def test_filter_by_type(client: AsyncClient, project_id: str):
    await client.post(f"/api/v1/projects/{project_id}/requirements", json={"title": "Func req", "req_type": "functional"})
    await client.post(f"/api/v1/projects/{project_id}/requirements", json={"title": "Security req", "req_type": "security"})
    resp = await client.get(f"/api/v1/projects/{project_id}/requirements?type=security")
...
async def test_update_requirement(client: AsyncClient, project_id: str):
    create = await client.post(f"/api/v1/projects/{project_id}/requirements", json={"title": "Original"})
    req_id = create.json()["id"]
    resp = await client.patch(f"/api/v1/projects/{project_id}/requirements/{req_id}", json={"title": "Updated"})
    assert resp.status_code == 200
...
async def test_delete_requirement(client: AsyncClient, project_id: str):
    create = await client.post(f"/api/v1/projects/{project_id}/requirements", json={"title": "To delete"})
    req_id = create.json()["id"]
